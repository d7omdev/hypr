...
-- Each shell action is bound twice: a `quickshell:*` global, then an exec
-- fallback for when the shell isn't running. Only the global carries the
-- description; the fallback is marked `[hidden]` so the cheatsheet lists the
-- action once. (Omitting the description is NOT enough — get_keybinds.py
-- synthesizes one from the dispatcher for any bind lacking it.)
hl.bind("SUPER + SUPER_L", hl.dsp.global("quickshell:searchToggleRelease"), { description = "Shell: Toggle search" })
hl.bind("SUPER + SUPER_R", hl.dsp.global("quickshell:searchToggleRelease")) -- # [hidden]
hl.bind("SUPER + SUPER_L", hl.dsp.exec_cmd(qsIsAlive .. " || pkill fuzzel || fuzzel")) -- # [hidden]
hl.bind("SUPER + SUPER_R", hl.dsp.exec_cmd(qsIsAlive .. " || pkill fuzzel || fuzzel")) -- # [hidden]

-- Internal: feeds the shell's workspace-number overlay, not a user action.
...
hl.bind("SUPER + B", hl.dsp.global("quickshell:sidebarLeftToggle")) -- # [hidden] alias of SUPER + A
hl.bind("SUPER + O", hl.dsp.global("quickshell:sidebarLeftToggle")) -- # [hidden] alias of SUPER + A
...
hl.bind("CTRL + ALT + Delete", hl.dsp.exec_cmd(qsIsAlive .. " || pkill wlogout || wlogout -p layer-shell")) -- # [hidden]
...
hl.bind("CTRL + SUPER + T", hl.dsp.exec_cmd(qsIsAlive .. " || " .. qsScripts .. "/colors/switchwall.sh")) -- # [hidden]
hl.bind(
	"CTRL + SUPER + R",
...
hl.bind("SUPER + SHIFT + A", hl.dsp.exec_cmd(qsIsAlive .. " || pidof slurp || " .. hyprScripts .. "/snip_to_search.sh")) -- # [hidden]
--# OCR
hl.bind(
	"SUPER + SHIFT + X",
...
hl.bind("SUPER + ALT + R", hl.dsp.global("quickshell:regionRecord"), { locked = true }) -- # [hidden] alias of SUPER + SHIFT + R
hl.bind("SUPER + ALT + R", hl.dsp.exec_cmd(qsIsAlive .. " || " .. qsScripts .. "/videos/record.sh"), { locked = true }) -- # [hidden]
...
hl.bind("CTRL + Print", hl.dsp.exec_cmd(grimhyprctl .. " - | wl-copy"), { locked = true, non_consuming = true }) -- # [hidden] second half of the CTRL + Print pair above
--# AI
hl.bind(
...
--# Zoom with keypad — aliases of SUPER + Minus/Equal above.
hl.bind("SUPER + code:82", function()
	zoomfunction(-0.3)
end, { repeating = true }) -- # [hidden]
...
-- The SUPER + SHIFT + <key> binds are canonical and carry the descriptions; the
-- XF86 media keys and mouse combos are aliases, hidden from the cheatsheet so
-- each action is listed once. More aliases live in custom/keybinds.lua.
hl.bind("SUPER + SHIFT + N", hl.dsp.exec_cmd(mediaNextCommand), { locked = true, description = "Media: Next track" })
hl.bind("XF86AudioNext", hl.dsp.exec_cmd(mediaNextCommand), { locked = true }) -- # [hidden]
...
hl.bind("SUPER + SHIFT + ALT + mouse:276", hl.dsp.exec_cmd(mediaNextCommand)) -- # [hidden]
hl.bind(
	"SUPER + SHIFT + B",
...
-- SUPER + mouse:274 was a second drag alias; it now resets cursor zoom in custom/keybinds.lua.
hl.bind("SUPER + mouse:273", hl.dsp.window.resize(), { mouse = true, description = "Window: Resize" })
-- Binds built in a loop have a key the cheatsheet parser cannot resolve
-- statically ("SUPER + " .. arrowkey[i]), so each loop is hidden and documented
-- by the `--#/#` pseudo-bind line above it instead.
--#/# hl.bind("SUPER + ←/↑/→/↓", hl.dsp.focus()) -- Window: Focus in direction
for i = 1, 4 do
...
--#/# hl.bind("SUPER + SHIFT + ←/↑/→/↓", hl.dsp.window.move()) -- Window: Move in direction
for i = 1, 4 do
	local arrowkey = { "Left", "Right", "Up", "Down" }
	local focusdir = { "l", "r", "u", "d" }
...
-- Nag, not an action: Alt+F4 stays free for Windows VMs, so it only explains itself.
...
hl.bind("SUPER + Semicolon", hl.dsp.layout("splitratio -0.1"), {
	repeating = true,
	description = "Layout: Shrink split ratio",
})
hl.bind("SUPER + Apostrophe", hl.dsp.layout("splitratio +0.1"), {
	repeating = true,
	description = "Layout: Grow split ratio",
})
--# Positioning mode
...
--#/# hl.bind("SUPER + ALT + 1…0", hl.dsp.window.move()) -- Window: Send to workspace 1-10
for i = 1, 10 do
...
--#/# hl.bind("SUPER + SHIFT + Scroll ↑/↓", hl.dsp.window.move()) -- Window: Send to workspace left/right (scroll)
-- SUPER + ALT + scroll is an alias of SUPER + SHIFT + scroll (same r-1/r+1 pair).
for i = 1, 4 do
...
--#/# hl.bind("SUPER + SHIFT + Page_↑/↓", hl.dsp.window.move()) -- Window: Send to workspace left/right
for i = 1, 2 do
	local keydirs = { "Up", "Down" }
	local prefix = { "r-", "r+" }
	hl.bind("SUPER + SHIFT + Page_" .. keydirs[i], hl.dsp.window.move({ workspace = prefix[i] .. "1" })) -- # [hidden]
end
for i = 1, 4 do
...
-- CTRL + SUPER + S dropped: it duplicated SUPER + S (toggle scratchpad) here and
-- SUPER + ALT + S (send to scratchpad) in custom/keybinds.lua. The key is free.

--##! Workspace
--# Switching
--#/# hl.bind("SUPER + 1…0", hl.dsp.focus()) -- Workspace: Focus 1-10
for i = 1, 10 do
...
-- CTRL + SUPER + ←/→ dropped: those keys now resize the scrolling-layout column
-- in custom/keybinds.lua. Workspace focus left/right is on SUPER + Page_↑/↓ below.
--#/# hl.bind("CTRL + SUPER + ALT + ←/→", hl.dsp.focus()) -- # [hidden] Focus busy left/right
for i = 1, 2 do
...
-- Bound literally rather than in a loop so the cheatsheet parser can resolve the
-- key and show the description.
hl.bind("SUPER + Page_Up", hl.dsp.focus({ workspace = "r-1" }), { description = "Workspace: Focus left" })
hl.bind("SUPER + Page_Down", hl.dsp.focus({ workspace = "r+1" }), { description = "Workspace: Focus right" })
hl.bind("CTRL + SUPER + Page_Up", hl.dsp.focus({ workspace = "r-1" })) -- # [hidden] alias of SUPER + Page_Up
hl.bind("CTRL + SUPER + Page_Down", hl.dsp.focus({ workspace = "r+1" })) -- # [hidden] alias of SUPER + Page_Down

--#/# hl.bind("SUPER + Scroll ↑/↓", hl.dsp.focus()) -- Workspace: Focus left/right (scroll)
for i = 1, 4 do
...
for i = 1, 2 do
	...
	local prefix = { "-1", "+1" }
	hl.bind("CTRL + SUPER + " .. key[i], hl.dsp.focus({ workspace = prefix[i] })) -- # [hidden]
end
hl.bind("CTRL + SUPER + Up", hl.dsp.focus({ workspace = "r-5" }), { description = "Workspace: Jump 5 back" })
hl.bind("CTRL + SUPER + Down", hl.dsp.focus({ workspace = "r+5" }), { description = "Workspace: Jump 5 forward" })
...
hl.define_submap("virtual-machine", function()
	...
	end, { submap_universal = true, description = "Session: Toggle virtual-machine submap" })
end)
...
hl.bind("SUPER + T", hl.dsp.exec_cmd(terminal)) -- # [hidden] alias of SUPER + Return
hl.bind("CTRL + ALT + T", hl.dsp.exec_cmd(terminal)) -- # [hidden] alias of SUPER + Return
